...
#' @name SWeeP
#' @description A vectorial comparative method to biological sequences (amino acid and nucleotide) and expression matrices.
#' The "Spaced Words Projection (SWeeP)" is a method for
#' representing biological sequences (amino acid and nucleotide) using compact vectors. SWeeP uses the
...
#' smaller randomly oriented orthonormal base (De Pierri, 2020). This way the resulting
...
#' @param bin       binary mode (TRUE), or count mode (FALSE) for HDV construction. Default is TRUE
...
#' @param transpose   If the rows correspond to the samples and the columns correspond to the genes (mtx pattern), use transpose=FALSE. If the columns correspond to the samples, use transpose=TRUE. Available only for inputs of the expression matrix or generic matrix type. The default setting is TRUE
...
#' @param norm   normalization via log. For counting mode, norm converts 0 to -1 and applies log() to non-zeros. For the binary case, it only converts 0 to -1. Default is TRUE
#' @param RNAseqdata  For RNAseq data. Apply the parameter `transpose=TRUE'. Default is FALSE.
#' @param ncores    Number of CPU cores used for parallel processing. Default is N-1, where N is the maximum.
...
#' @name SWeePlite
#' @description A vectorial comparative method to biological sequences (amino acid and nucleotide) and expression matrices.
#' The "Spaced Words Projection version lite" (SWeePlite) is a method for
#' representing biological sequences using compact vectors. SWeePlite uses the
#' spacedwords concept by scanning sequences and generating indices
#' to create a higherdimensional matrix of occurrences that is later projected into a
#' smaller randomly oriented orthonormal base (De Pierri, 2020). This way the resulting
#' matrix  will conserve the comparational data but will have a selectable size.
...
#' @param concatenate   defines whether to treat each sequence individually or to concatenate them into a single sequence
#'                      Available only for inputs in biological sequence format. The default is FALSE.
#' @param norm   normalization option: 'none', 'log', 'logNeg'.  Default is 'none'
#' @param RNAseqdata  For RNAseq data. Apply the parameter `transpose=TRUE'. Default is FALSE.
#' @param ncores    Number of CPU cores used for parallel processing. Default is N-1, where N is the maximum.
